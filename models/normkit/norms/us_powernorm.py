...
class PowerFunction_PN(torch.autograd.Function):
    @staticmethod
    ...
    def forward(ctx, x, weight, bias, running_phi, eps, afwd, abkw, ema_gz, \
        ...
        ctx.save_for_backward(z, var, weight, ema_gz, running_phi)
    ...
    def backward(ctx, grad_output):
        ...
        z, var, weight, ema_gz, running_phi = ctx.saved_variables
        ...
        if current_iter >= warmup_iters:
            gx = 1. / torch.sqrt(running_phi + eps) * approx_grad_g
        else:
...
class PowerNorm(nn.Module):
    ...
    def forward(self, input, pad_mask=None, is_encoder=False):
        ...
        input_shape = input.size() #B, C, T
        input = input.reshape(input.size(0), self.num_features, -1) #B, C, T, 1
